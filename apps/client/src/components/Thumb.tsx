...
export default function Thumb({ className = "w-full h-24 ", gameSlug }: { className?: string; gameSlug?: string }) {
  const config = {
    "color-match-blitz": { bg: "var(--color-accent)", ink: "#171a35", icon: "pixelarticons:zap", badge: "BLITZ" },
    "color-match-chaos": { bg: "var(--color-danger)", ink: "#171a35", icon: "pixelarticons:reload", badge: "CHAOS" },
    "descarte-criollo": { bg: "var(--color-warning)", ink: "#171a35", icon: "pixelarticons:sword", badge: "CRIOLLO" },
  }[gameSlug ?? ""];
  ...
  return (
    ...
    >
      {config ? (
        ...
        <Icon icon="pixelarticons:dice" width={28} height={28} />
      )}
      {config?.badge && (
